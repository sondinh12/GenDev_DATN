...
@section('content')
    ...
    <div class="container-fluid px-4">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h4 class="mb-0">
                    <i class="fas fa-edit text-primary me-2"></i> Chỉnh sửa Banner
                </h4>
            </div>
            ...
            <div class="card-body">
                <form method="POST" action="{{ route('banner.update', $banner->id) }}" enctype="multipart/form-data">
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <select name="status" class="form-select @error('status') is-invalid @enderror" required>
                                    <option value="unused"
                                        {{ old('status', $banner->status) == 'unused' ? 'selected' : '' }}>
                                        Chưa được sử dụng
                                    </option>
                                    <option value="using"
                                        {{ old('status', $banner->status) == 'using' ? 'selected' : '' }}>
                                        Đang được sử dụng
                                    </option>
                                    <option value="expired"
                                        {{ old('status', $banner->status) == 'expired' ? 'selected' : '' }}>
                                        Đã hết hạn
                                    </option>
                                </select>
                                @error('status')
                    ...
                    <div id="static-image" class="mb-4">
                        ...
                        <div class="mb-3">
                            @if ($banner->image)
                                <img src="{{ asset('storage/' . $banner->image) }}" class="img-thumbnail"
                        ...
                        <div id="existing-images" class="mb-4">
                            ...
                            @forelse($imgs as $i => $img)
                                <div class="mb-3 p-3 border rounded d-flex bg-light">
                                    <img src="{{ asset('storage/' . $img) }}" class="img-thumbnail me-3"
            ...
            document.querySelectorAll('input[name="delete_images[]"]').forEach(cb => {
                cb.addEventListener('change', function() {
                    const toDelete = document.querySelectorAll(
                        'input[name="delete_images[]"]:checked').length;
                    if (total - toDelete < 2) {
                        Swal.fire({
                            icon: 'error',
                            title: 'Không thể xóa',
