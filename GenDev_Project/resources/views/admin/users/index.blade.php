                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="d-flex align-items-center">
                                            <img src="{{ $user->avatar ? asset('storage/' . $user->avatar) : asset('storage/images/default-avatar.png') }}"
                                                alt="{{ $user->name }}" class="rounded-circle me-2" width="40" height="40" style="object-fit: cover;">
                                            <div>
                                    ...
                                    <td>
                                        ...
                                        @endphp
                                        <span class="badge bg-{{ $roleLabel['class'] }}">{{ $roleLabel['label'] }}</span>
                                    </td>
                                    <td>
                                        @if($user->status == 1)
                                    ...
                                    <td class="text-center">
                                        ...
                                        @if($currentUser->role === 0 && $currentUser->id !== $user->id)
                                            <!-- Sửa vai trò -->
                                        ...
                                        @if($currentUser->id !== $user->id && $currentUser->role < $user->role)
                                            @if($user->status == 1)
                                                <form action="{{ route('admin.users.ban', $user->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Bạn có chắc muốn cấm người dùng này?')">
                                                    @csrf
                                            ...
                                            @else
                                                <form action="{{ route('admin.users.unban', $user->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Bạn có chắc muốn gỡ cấm người dùng này?')">
                                                    @csrf
