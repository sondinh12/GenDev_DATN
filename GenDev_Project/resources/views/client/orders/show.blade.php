        ...
        <div class="order-summary mb-4">
            ...
            @foreach ($order->orderDetails as $detail)
                <div class="product-item d-flex justify-content-between">
                    <div class="d-flex">
                        <img src="{{ asset('storage/' . $detail->product->image) }}" alt="" width="80"
                            height="80" class="me-3 rounded border">
                        <div>
                            ...
                            @if ($detail->variant && $detail->variant->variantAttributes->count())
                                <div class="text-muted small mt-1">
                                    @foreach ($detail->variant->variantAttributes as $attr)
                                        {{ $attr->attribute->name ?? '' }}: {{ $attr->value->value ?? '' }}@if (!$loop->last)
                                            ,
                                        @endif
                                    @endforeach
                                </div>
            ...
            <div class="text-end mt-4">
                ...
                @if ($order->productCoupon)
                    ...
                    <div class="text-success">Giảm sản phẩm: -{{ number_format($order->product_discount, 0, ',', '.') }} đ
                    </div>
                @endif
                ...
                @if ($order->shippingCoupon)
                    ...
                    <div class="text-success">Giảm phí vận chuyển:
                        -{{ number_format($order->shipping_discount, 0, ',', '.') }} đ</div>
                @endif
        ...
        <div class="d-flex gap-2">
            ...
            @if ($order->status === 'shipped')
                <form action="{{ route('client.orders.complete', $order->id) }}" method="POST"
                    onsubmit="return confirm('Bạn xác nhận đã nhận hàng và muốn hoàn thành đơn này?');">
                    @csrf
                    @method('PUT')
