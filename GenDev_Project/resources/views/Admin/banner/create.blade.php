        ...
        <div class="row mb-4">
            <div class="col-12">
                <div class="d-flex justify-content-between align-items-center">
                    <h4 class="mb-0">
                        <i class="fas fa-image text-primary me-2"></i>Thêm mới Banner
                    </h4>
            ...
            <div class="card-body">
                <form method="POST" action="{{ route('banner.store') }}" enctype="multipart/form-data" novalidate>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <select name="type" id="type"
                                    class="form-select @error('type') is-invalid @enderror" onchange="handleTypeChange()"
                                    required>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <select name="status" class="form-select @error('status') is-invalid @enderror" required>
                                    <option value="unused" {{ old('status') == 'unused' ? 'selected' : '' }}>Chưa được sử dụng
                                    </option>
                                    <option value="using" {{ old('status') == 'using' ? 'selected' : '' }}>Đang được sử dụng
                                    </option>
                    ...
                    <div class="form-group mb-4" id="static-image">
                        <label class="form-label fw-semibold">Ảnh banner<span class="text-danger">*</label>
                        <input type="file" name="image" class="form-control @error('image') is-invalid @enderror"
                            accept="image/*" onchange="previewImage(this,'static-preview')">
                        @error('image')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                        <div class="mt-2">
                            <img id="static-preview" src="#" alt="Preview" class="img-thumbnail d-none"
                    ...
                    <div class="form-group mb-4" id="dynamic-images" style="display:none;">
                        ...
                        @if ($errors->has('images.*'))
                            @foreach ($errors->get('images.*') as $idx => $msgs)
                                @foreach ($msgs as $msg)
                                    <div class="text-danger small mb-1">Ảnh #{{ $idx + 1 }}: {{ $msg }}
                                    </div>
                                @endforeach
                            @endforeach
                        @endif

                        <button type="button" class="btn btn-outline-primary me-2" onclick="addImageInput()">
